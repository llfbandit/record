...
extension AudioRecordingDelegate {
  func initAVAudioSession(config: RecordConfig, manageAudioSession: Bool) throws {
    ...
    do {
      try audioSession.setPreferredSampleRate(
        (config.sampleRate <= 48000) ? Double(config.sampleRate) : 48000.0)
    } catch {
    ...
    if #available(iOS 14.5, *) {
      do {
        try audioSession.setPrefersNoInterruptionsFromSystemAlerts(
          config.audioInterruption == AudioInterruptionMode.none)
      } catch {
        throw RecorderError.error(
          message: "Failed to start recording",
    ...
    if manage {
      do {
        try audioSession.setCategory(
          .playAndRecord, options: AVAudioSession.CategoryOptions(config.iosConfig.categoryOptions))
      } catch {
        throw RecorderError.error(
          message: "Failed to start recording",
      ...
      do {
        ...
        throw RecorderError.error(
          message: "Failed to start recording", details: "setActive: \(error.localizedDescription)")
      }
    }

    do {
      let newPreferredInputNumberOfChannels = min(
        config.numChannels, audioSession.maximumInputNumberOfChannels)
    ...
    do {
      ...
      throw RecorderError.error(
        message: "Failed to start recording", details: "setInput: \(error.localizedDescription)")
    }
  ...
  func deactivateAudioSession() {
    guard let conf = config, conf.iosConfig.manageAudioSession else {
      return
    }
    ...
    do {
      ...
      // Silently fail - audio session deactivation is best effort
    }
  }
